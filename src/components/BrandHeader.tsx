...
export function BrandHeader({ locale, onLocaleChange }: BrandHeaderProps) {
  return (
    <div className="relative flex items-center justify-between border-b border-[#dbe1e9] bg-[#ffffff] px-5 py-4 md:px-7 md:py-4">
      <div className="flex items-center gap-3">
        ...
        <p className="m-0 text-[12px] font-semibold uppercase tracking-[0.2em] text-[#7f6b46]">{copy[locale].serviceCenter}</p>
      </div>
      <div className="flex items-center gap-2">
        <div className="flex items-center rounded-full border border-[#cfe1d2] bg-[#eef8f0] px-2.5 py-1.5" aria-label={copy[locale].online} title={copy[locale].online}>
          <StatusIcon />
        </div>
